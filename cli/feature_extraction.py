...
class ArticleVector:
    ...
    def url_ending_index(self):
        '''
        returns 1 if url has reputable ending, 0 otherwise and also 0 if the url is empty or not provided.
        '''
        reputable_endings = ['com', 'gov', 'org']
        ending = tldextract.extract(self.url).suffix
        return int(ending in reputable_endings) if self.url else 0

    def dot_gov_ending_index(self):
        '''
        returns 1 if url ends in .gov, 0 otherwise and also 0 if the url is empty or not provided.
        '''
        ending = tldextract.extract(self.url).suffix
        return int(ending == 'gov') if self.url else 0
